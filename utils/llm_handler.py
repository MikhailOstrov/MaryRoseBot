...
async def get_response(user_text: str, chat_id: int) -> str:
    
    prompt = f'''Определи намерение пользователя по сообщению и обработай текст в зависимости от результата.
    ...
    Также учитывай контекст, если ты определяешь намерения пользователя под цифрой 2: {format_chat_history_for_prompt(chat_id)}
