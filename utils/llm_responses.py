...
async def llm_response(user_text: str) -> str:
    ...
    **Текущая дата: {date.strftime('%d.%m.%Y')}**
    **Учитывай текущую дату при обработке запросов, связанных с событиями или встречами.** Если пользователь использует относительные указания времени (например, "завтра", "послезавтра", "на следующей неделе"), конвертируй их в точные даты, основываясь на сегодняшней дате.
    ...
    Если пользователь хочет добавить информацию или же просит поставить напоминание, то тебе нужно лишь занести её в базу знаний, не меняя текст пользователя, дай ответ в формате:
